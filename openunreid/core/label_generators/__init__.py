...
from ...utils.dist_utils import synchronize, get_dist_info, \
                            broadcast_tensor, broadcast_value
...
class LabelGenerator(object):
    ...
    def __init__(
        self,
        cfg,
        models, # list of models, e.g. MMT has two models
        verbose = True
    ):
        super(LabelGenerator, self).__init__()

        assert ('PSEUDO_LABELS' in cfg.TRAIN), \
        ...
        self.data_loaders, self.datasets = build_val_dataloader(cfg, for_clustering=True)
    ...
    def __call__(
        ...
        **kwargs
        ...
        for idx, (data_loader, dataset) in enumerate(zip(self.data_loaders, self.datasets)):
            ...
            if (self.rank==0):
                ...
                labels, centers, num_classes, indep_thres = \
                            self.__factory[self.cluster_type](
                                                self.cfg,
                                                all_features,
            ...
            if (self.world_size>1):
                ...
                if (self.cfg.TRAIN.PSEUDO_LABELS=='dbscan' and len(self.cfg.TRAIN.PSEUDO_LABELS.eps)>1):
        ...
        if self.verbose:
            dataset_names = [list(self.cfg.TRAIN.datasets.keys())[i] for i in self.cfg.TRAIN.unsup_dataset_indexes]
    ...
    def print_label_summary(self, epoch, pseudo_labels, dataset_name):
        ...
        print('\n==> Statistics for {} on epoch {}: {} clusters, {} un-clustered instances, {} unused instances\n'
                    .format(dataset_name, epoch, clu_num, unclu_ins_num, unused_ins_num))
