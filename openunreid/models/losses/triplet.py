...
def _batch_hard(mat_distance, mat_similarity, return_indices=False):
	...
	sorted_mat_distance, positive_indices = torch.sort(mat_distance + (-9999.) * (1 - mat_similarity), dim=1, descending=True)
	hard_p = sorted_mat_distance[:, 0]
	hard_p_indice = positive_indices[:, 0]
	sorted_mat_distance, negative_indices = torch.sort(mat_distance + (9999.) * (mat_similarity), dim=1, descending=False)
...
class SoftSoftmaxTripletLoss(TripletLoss):
	...
	def forward(self, results, targets, results_mean):
		...
		emb1 = results['pooling']
		emb2 = results_mean['pooling']
