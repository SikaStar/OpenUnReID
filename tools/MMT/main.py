...
from openunreid.apis import test_reid, BaseRunner, batch_processor, val_reid
...
class MMTRunner(BaseRunner):
    ...
    def train_step(self, iter, batch):
        ...
        results_1['prob'] = results_1['prob'][:,:self.train_loader.loader.dataset.num_pids]
        results_1_mean['prob'] = results_1_mean['prob'][:,:self.train_loader.loader.dataset.num_pids]
        results_2['prob'] = results_2['prob'][:,:self.train_loader.loader.dataset.num_pids]
        results_2_mean['prob'] = results_2_mean['prob'][:,:self.train_loader.loader.dataset.num_pids]

        total_loss = 0
        meters = {}
        for key in self.criterions.keys():
            if (key=='soft_entropy'):
                loss = self.criterions[key](results_1, results_2_mean) + \
                        self.criterions[key](results_2, results_1_mean)
            elif (key=='soft_softmax_triplet'):
                loss = self.criterions[key](results_1, targets, results_2_mean) + \
                        self.criterions[key](results_2, targets, results_1_mean)
            else:
                loss = self.criterions[key](results_1, targets) + \
                        self.criterions[key](results_2, targets)
